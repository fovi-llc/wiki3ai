...
Before the model can be downloaded, there must be a[user interaction](https://developer.chrome.com/docs/ai/get-started#user-activation), such as a click, tap, or key press.

If the response was`downloadable`or`downloading`, the model and APIs are available but must be downloaded before you can use the features. The user must[interact with the page](https://developer.chrome.com/docs/ai/get-started#user-activation)(such as a click, tap, or key press) for a download to be permitted.

To download and instantiate the model, call the[`create()`function](https://developer.chrome.com/docs/ai/prompt-api#create_a_session).  
